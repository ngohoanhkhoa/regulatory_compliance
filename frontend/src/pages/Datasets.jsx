...
function kindMeta(kind, t) {
  if (kind === "documents") {
    return { label: t("Documents"), variant: "info", icon: FileText };
  }
  return { label: t("Regulatory"), variant: "success", icon: Database };
}
  ...
  return (
    <div className="data-page">
      ...
      <h3 className="documents-section-title">
        ...
        {t("Regulatory texts")} ({regulatory.length})
      </h3>
      {regulatory.length === 0 ? (
        <div className="data-empty">
          <Database className="w-10 h-10 mx-auto mb-3 opacity-50" />
          <p>{t("No regulatory datasets yet.")}</p>
        </div>
      ) : (
